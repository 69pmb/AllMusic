...
/**
 * Class to contains constants to calculate the score of Composition. Score calculation: {@code max
 * * Log10(max) * Log10((median*2)/currentRank + 3)}. Created by PBR on 19 juin 2018.
 */
public class Score {

  /**
   * Max is the biggest file size for songs files. LogMaxSong is equals to {@code Log10(max) * max}.
   */
  private BigDecimal logMaxSong;

  /**
   * Median is the ranking median of the song files. DoubleMedianSong is equals to the double of
   * median.
   */
  private BigDecimal doubleMedianSong;

  /**
   * Max is the biggest file size for album files. LogMaxSong is equals to {@code Log10(max) * max}.
   */
  private BigDecimal logMaxAlbum;

  /**
   * Median is the ranking median of the album files. DoubleMedianSong is equals to the double of
   * median.
   */
