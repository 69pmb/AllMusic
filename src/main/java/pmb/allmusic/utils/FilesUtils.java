...
public final class FilesUtils {
      ...
      if (new File(filePath).exists()) {
        ...
        try {
          ...
          throw new MajorException(
              "Le chemin de Notepad++ dans le fichier de config est incorrect.", e);
        }
      } else {
  ...
  public static Optional<String> buildTxtFilePath(String fileName, String auteur) {
    ...
    return Stream.concat(
            Stream.of(pathShort),
            Stream.of(Constant.SONG_FOLDER, Constant.ALBUM_FOLDER, Constant.YEAR_FOLDER)
                .map(buildPath::apply))
  ...
  public static Optional<String> buildXmlFilePath(String fileName) {
    ...
    path +=
        StringUtils.endsWithIgnoreCase(fileName, MyConstant.XML_EXTENSION)
            ? ""
  ...
  public static void writeMapInTxtFile(File file, Map<String, String> map) {
    ...
    String lines =
        pmb.my.starter.utils.MyFileUtils.readFile(file).stream()
            .filter(
                line -> {
                  if (StringUtils.startsWith(line, Constant.IMPORT_PARAMS_PREFIX)
                      && isFirstLine.get()) {
    ...
    pmb.my.starter.utils.MyFileUtils.writeFile(
        file, List.of(Constant.IMPORT_PARAMS_PREFIX + s, lines));
