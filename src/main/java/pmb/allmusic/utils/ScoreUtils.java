...
public final class ScoreUtils {
  ...
  public static List<Double> getDecileLimit(List<Composition> list) {
    Map<Composition, Long> collectedMap =
        list.stream()
    ...
    for (int i = 0; i < size; i++) {
      value[i] =
          q[i].evaluate(
              collectedMap.values().stream().mapToDouble(Long::doubleValue).toArray(), prob[i]);
    }
   ...
   *   <li>ALL_TIME: score * 2
   *   <li>LONG_PERIOD: score * 1.5
   *   <li>DECADE: score * 1.3
   *   <li>YEAR: score / 1.5
   *   <li>THEME: score / 1.3
   *   <li>GENRE: score * 1
   *   <li>MISCELLANEOUS: score * 1
   * </ul>
   *
   ...
   */
  private static BigDecimal getFileScore(
      BigDecimal logMax, BigDecimal doubleMedian, Fichier fichier) {
    BigDecimal points;
      ...
      points =
          BigDecimal.valueOf(
                  Math.log10(
                      doubleMedian
                          .divide(
                              BigDecimal.valueOf(fichier.getClassement()), 10, RoundingMode.HALF_UP)
  ...
  /**
   * Calculates the score for a composition, it's depending on the {@link RecordType} of the
   * composition. It's the sum of the score of its files.
   *
  ...
  /**
   * Calculates the {@link Score} logMax for the given type. Max is the biggest size of all files.
   * LogMax is {@code Log10(max) * max}.
   *
   * @param list {@code List<Composition>}
  ...
  public static BigDecimal getLogMax(List<Composition> list) {
    ...
    BigDecimal max =
        new BigDecimal(rankList.parallelStream().mapToInt(Integer::intValue).max().orElse(1));
    // log10(max)*max
    return BigDecimal.valueOf(Math.log10(max.doubleValue())).multiply(max);
  }

  /**
   * Calculates the {@link Score} doubleMedian. The median of all files ranking multiplied by 2.
   *
   * @param list {@code List<Composition>}
   * @return {@link BigDecimal}
   */
    ...
    return BigDecimal.valueOf(
            VariousUtils.median(
                list.parallelStream()
                    .map(Composition::getFiles)
                    .flatMap(List::stream)
                    .map(f -> BigDecimal.valueOf(f.getClassement()))
                    .collect(Collectors.toList())))
