...
public final class FichierUtils {
   ...
   * @param getComposition if true recovers the associated composition for each file
   * @param lineNumber if true add a columns for line number
   * @return Vector<Vector<Object>> the list converted
   */
  public static Vector<Vector<Object>> convertCompositionListToFichierVector(
      List<Composition> compoList, boolean getComposition, boolean lineNumber) {
    LOG.debug(
        "Start convertCompositionListToFichierVector, getComposition: {}, lineNumber: {}",
        getComposition,
        lineNumber);
    return compoList.parallelStream()
        .map(
            c ->
                        ...
                        f -> {
                          ...
                          List<Composition> importXML =
                              ImportXML.importXML(
                                  Constant.getXmlPath()
                                      + f.getFileName()
                                      + MyConstant.XML_EXTENSION);
                          Composition compo = new Composition();
                          if (getComposition) {
                            compo =
                                CompositionUtils.findByUuid(importXML, c.getUuids())
                                    .orElseGet(
                                        () -> {
                                          LOG.warn(
                                              "No result when searching composition by its file: {}, {}, {}",
                                              f,
                                              c.getArtist(),
                                              c.getTitre());
                                          return c;
                          ...
                          MiscUtils.addElements(
                              v,
                              f.getPublishYear(),
                              f.getCategorie().getValue(),
                              f.getRangeDateBegin() + " - " + f.getRangeDateEnd(),
                              getPercentOfDeleted(f, importXML));
                          if (!getComposition) {
                          ...
                          if (getComposition) {
                            MiscUtils.addElements(
                                v,
                                ScoreUtils.getCompositionScore(compo),
                                f.getClassement(),
                                Boolean.toString(compo.isDeleted()),
                                VariousUtils.uuidsToString(compo.getUuids()));
                          }
  ...
  /**
   * Modifie un fichier, dans le fichier final.xml, dans son fichier xml et renomme si besoin les
   * fichiers XML et TXT.
   *
   ...
   */
  public static Fichier modifyFichier(
      String fileName,
      String newFileName,
      String newPublish,
      String newRange,
    ...
    compoList.stream()
        .forEach(
            modifyOneFichier(
                fileName, newFileName, newPublish, newRange, newCat, newSize, newSorted));
    ...
    finalList.stream()
        .filter(
            c -> c.getFiles().stream().anyMatch(f -> StringUtils.equals(f.getFileName(), fileName)))
        .forEach(
            modifyOneFichier(
                fileName, newFileName, newPublish, newRange, newCat, newSize, newSorted));
    try {
      ...
      throw new MajorException(
          "Erreur lors de la modification d'une composition dans le fichier final", e);
    }
    // Renomme le fichier txt
    String txtPath =
        FilesUtils.buildTxtFilePath(fileName, result.getAuthor())
            .orElseThrow(
                () ->
                    new MajorException(
                        "Can't build txt file path of: "
                            + fileName
                            + " with author: "
                            + result.getAuthor()));
    String newTxt =
        StringUtils.substringBeforeLast(
                StringUtils.substringBeforeLast(txtPath, MyConstant.TXT_EXTENSION), MyConstant.FS)
            + MyConstant.FS
            + newFileName
    ...
    if (StringUtils.startsWith(firstLine, Constant.IMPORT_PARAMS_PREFIX)) {
      ...
      value.put(
          ImportPanel.IMPORT_PARAM_SORTED,
          StringUtils.equalsIgnoreCase(newSorted, "oui")
   ...
   */
  public static Consumer<Composition> modifyOneFichier(
      String fileName,
      String newFileName,
      String newPublish,
      String newRange,
    ...
    return c -> {
      List<Fichier> list =
          c.getFiles().stream()
              .filter(f -> StringUtils.equals(f.getFileName(), fileName))
      ...
      list.forEach(
          fichier -> {
            ...
            fichier.setSorted(
                StringUtils.equalsIgnoreCase(newSorted, "oui") ? Boolean.TRUE : Boolean.FALSE);
          });
    };
  }
  ...
  public static Predicate<Fichier> filterFichier(Vector<Object> row, ColumnIndex index) {
    return f ->
        !(f.getClassement().equals(row.get(index.get(Index.RANK)))
