...
public final class CsvFile {
  ...
  public static String exportCsv(
      String filename, List<List<String>> csv, List<SortKey> sortKeys, String[] header) {
    ...
    List<Boolean> matrixHeaders =
        headerList.stream().map(StringUtils::isNotBlank).collect(Collectors.toList());
    // Writing
    try (CSVWriter csvWriter =
        new CSVWriter(
            new OutputStreamWriter(new FileOutputStream(name), MyConstant.ANSI_ENCODING),
            Constant.getCsvSeparator(),
    ...
    try (BufferedReader br =
      ...
      result =
          new CsvToBeanBuilder<T>(br)
              .withType(returnType)
              .withSeparator(Constant.getCsvSeparator())
              .build()
              .parse();
    } catch (IOException e) {
   ...
   */
  public static <T> void exportBeanList(
      File csvFile, List<T> beans, MappingStrategy<T> mappingStrategy, boolean append) {
    LOG.debug("Start exportBeanList: {}", csvFile.getAbsolutePath());
    try (OutputStreamWriter writer =
        new OutputStreamWriter(new FileOutputStream(csvFile, append), MyConstant.ANSI_ENCODING)) {
