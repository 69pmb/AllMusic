...
public final class ImportFile {
   ...
   * @param reverseArtist si l'artiste est par exemple: {@literal Beatles, The} et doit etre
   *     retourné
   ...
   */
  public static List<Composition> getCompositionsFromFile(
      File file,
      Fichier fichier,
      RecordType type,
      String separator,
    ...
    if (!StringUtils.equalsIgnoreCase(fichier.getAuthor(), Constant.VARIOUS_AUTHOR)
        && (!StringUtils.startsWithIgnoreCase(
                fichier.getFileName(), fichier.getAuthor() + Constant.FILE_NAME_SEPARATOR)
            || !StringUtils.endsWithIgnoreCase(
                fichier.getFileName(),
                Constant.FILE_NAME_SEPARATOR + String.valueOf(fichier.getPublishYear())))) {
      result.add(
          "### Incorrect file name: "
              + fichier.getFileName()
              + ", it must start with author and end with publish year");
    }
    LOG.debug(result);
  ...
  private static void getCompositionFromOneLine(
    ...
    Integer rank =
        setArtistAndTitreGetRank(composition, line, upper, split, fichier.getSorted(), i);
    ...
    composition.setArtist(
        WordUtils.capitalizeFully(
            StringUtils.removeEnd(StringUtils.removeStart(composition.getArtist(), "\""), "\"")));
    composition.setTitre(
        WordUtils.capitalizeFully(
            StringUtils.removeEnd(StringUtils.removeStart(composition.getTitre(), "\""), "\"")));
    if (parenthese) {
  ...
  private static Integer setArtistAndTitreGetRank(
   ...
   */
  private static String[] splitLineWithSeparator(
      String line, String separator, List<String> result, Integer lineNb) {
    ...
    if (split.length > 2) {
      ...
      if (StringUtils.countMatches(line, newSep) > 1) {
        result.add(
            "### Error Size (" + split.length + ") for: " + line + LOG_NUMBER + (lineNb - 1));
      }
    }
    return split;
  }

  private static void removeParentheseFromTitreAndArtist(
      List<String> result, String line, Integer lineNumber, Composition compo) {
   ...
   */
  public static Integer determineSize(
      Fichier fichier, List<String> randomLines, String absolutePath) {
  ...
  private static Integer determineSizeSorted(
      Fichier fichier, List<String> randomLines, Integer res) {
    Integer result = res;
    // TODO
  ...
  private static void determineYearsDecade(Fichier file, String[] split, List<String> date) {
    ...
    if (decadeMatch.size() == 1) {
      ...
      if (date.size() > 1) {
        res =
            date.stream().filter(s -> !s.matches(Constant.YEAR_AT_THE_END)).findFirst().orElse("");
      } else if (date.size() == 1) {
        res = date.get(0);
      }
  ...
  public static String getSeparator(String line) {
    ...
    Predicate<String> isSuitableSeparator =
        sep ->
            !StringUtils.isAlphanumeric(sep)
                && sep.length() == 1
                && !StringUtils.equals("-", sep)
                && !Arrays.asList(Constant.getNotSeparators()).contains(sep);
    List<String> splits = Arrays.asList(StringUtils.split(line, null));
    return splits.stream()
        .filter(isSuitableSeparator)
        .findFirst()
        .orElseGet(
            () ->
                splits.stream()
                    .map(split -> StringUtils.substring(split, 0, 1))
                    .filter(isSuitableSeparator)
                    .findFirst()
                    .orElseGet(
                        () ->
                                ...
                                .map(
                                    split ->
                                        StringUtils.substring(
                                            split, split.length() - 1, split.length()))
                                .filter(isSuitableSeparator)
                                .findFirst()
                                .orElse("-")));
  }
  ...
  /**
   * Retourne dans l'ordre les 3 premières lignes, une ligne au hasard et les 2 dernières lignes du
   * fichier donné et la derniere ligne non vide.
   *
  ...
  public static long countCharacter(File file, String character) {
    ...
    long result =
        MyFileUtils.readFile(file).stream()
            .filter(line -> StringUtils.contains(line, character))
            .count();
   ...
   * @param validLine true on compte seulement les lignes valides, plus longue que 5 caractères et
   *     non commentées, false on compte toutes les lignes.
   * @return un nombre
   */
  public static int countLines(String filename, boolean validLine) {
    return (int)
        MyFileUtils.readFile(filename).stream()
            .filter(line -> (validLine && isValidLine(line)) || (!validLine && line != null))
            .count();
  }
}
