...
/**
 * Class describing how to parse csv file when using {@code OpenCsv}. This is a custom class to mix
 * Column position mapping and Header column name mapping.
 *
...
public class CustomColumnPositionMappingStrategy<T> extends ColumnPositionMappingStrategy<T> {
  ...
  public CustomColumnPositionMappingStrategy(Class<? extends T> type, String[] columnMapping) {
    ...
    if (columnMapping != null) {
      ...
      this.setColumnMapping(
          FieldUtils.getFieldsListWithAnnotation(type, CsvBindByName.class).stream()
              .map(Field::getName)
  ...
  /**
   * This method returns an empty array. The column position mapping strategy assumes that there is
   * no header, and thus it also does not write one, accordingly.
   *
  ...
  public String[] generateHeader(T bean) throws CsvRequiredFieldEmptyException {
    ...
    List<Field> fields =
        FieldUtils.getFieldsListWithAnnotation(bean.getClass(), CsvBindByName.class);
   ...
   *
   * <p>This method is called by {@link #loadFieldMap()} when at least one relevant annotation is
   * found on a member variable.
   */
  @Override
  ...
  private void handleBindByPosition(Class<?> localType, Field localField) {
    ...
    fieldWriteLocale =
        annotation.writeLocaleEqualsReadLocale() ? fieldLocale : annotation.writeLocale();
    capture = annotation.capture();
    format = annotation.format();
    CsvConverter converter =
        determineConverter(localField, localField.getType(), fieldLocale, fieldWriteLocale, null);
  ...
  private void handleBindByName(int i, Field localField) {
    ...
    fieldMap.put(
        i,
        new BeanFieldSingleValue<>(
            localField.getType(), localField, required, errorLocale, converter, capture, format));
  }
  ...
  private void handleBindAndJoinByPosition(Class<?> localType, Field localField) {
    ...
    CsvConverter converter =
        determineConverter(localField, elementType, fieldLocale, fieldWriteLocale, joinConverter);
    fieldMap.putComplex(
        annotation.position(),
        new BeanFieldJoinIntegerIndex<>(
  ...
  private void handleBindAndSplitByPosition(Class<?> localType, Field localField) {
    ...
    String capture;
    String format;
    CsvBindAndSplitByPosition annotation =
        localField.getAnnotation(CsvBindAndSplitByPosition.class);
    required = annotation.required();
    fieldLocale = annotation.locale();
    fieldWriteLocale =
        annotation.writeLocaleEqualsReadLocale() ? fieldLocale : annotation.writeLocale();
    ...
    CsvConverter converter =
        determineConverter(localField, elementType, fieldLocale, fieldWriteLocale, splitConverter);
    fieldMap.put(
        annotation.position(),
        new BeanFieldSplit<>(
            localType,
  ...
  private void handleCustomBindByPosition(Class<?> localType, Field localField) {
    ...
    Class<? extends AbstractBeanField<T, Integer>> converter =
        (Class<? extends AbstractBeanField<T, Integer>>) annotation.converter();
