...
public final class CleanFile {
  ...
  /**
   * Deletes all lines which don't contain the separator or are too long, with the possibility to
  ...
  /**
   * Returns line if it contains given separator (or if the separator is blank) or if beginning by a
   * number for sorted files and then removes character at the begin or at the end of the line.
   *
  ...
  private static String clearLine(
      String line, String separator, String characterToRemove, boolean isBefore, boolean isSorted) {
    String newLine = line;
  ...
  /**
   * Mofifie les fichiers spécifiés: Remplace suivant le fichier modif.txt des caractères par
   * d'autres. Supprime tous les diacritiques.
   *
   * @param folder a folder or a file
   * @param isCompleteDirectory if true processes all files in folder, otherwise only given file
  ...
  public static void miseEnForme(File folder, boolean isCompleteDirectory, List<String> result) {
    ...
    files =
        files.stream()
            .filter(f -> !StringUtils.equals(Constant.getFinalFile(), f.getName()))
            .collect(Collectors.toList());
   ...
   */
  private static String formattingAndAppendingLine(
      Set<Entry<String, String>> rules, AtomicBoolean modify, String name, String line) {
  ...
  /**
   * Fetches the modif file where unwanted characters and their substitutes are describes.
   *
   * @return an Entry of a Set with key unwanted characters and value its substitute
   */
  ...
  public static String buildGeneratedFilePath(File file, String suffix) {
    return file.getParentFile().getAbsolutePath()
        + MyConstant.FS
        + StringUtils.substringBeforeLast(file.getName(), MyConstant.DOT)
        + suffix
