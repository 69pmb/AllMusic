...
/** Wrapper around {@link JTable} to handle generically sorting, popup, model and initialization. */
public class MyTable implements Serializable {
  ...
  /** Constructor of {@link MyTable}. Sets default value for its table (header, design, sorting). */
  public MyTable() {
    table = new JTable();
  ...
  public void updateTable(Vector<Vector<Object>> data, SortKey defaultSorting, boolean scrollTop) {
    ...
    table
        .getRowSorter()
        .setSortKeys(
            Collections.singletonList(new RowSorter.SortKey(getSortedColumn(), getSortOrder())));
    ...
    List.of(Index.PERCENT_DELETED, Index.SCORE_DELETED).stream()
        .filter(index::has)
        .forEach(
            column ->
                ((TableRowSorter<?>) table.getRowSorter())
                    .setComparator(index.get(column), VariousUtils.comparePercentage));

    // Hide technical columns
    deletedColumnIndex.set(0);
    List.of(Index.DECILE, Index.DELETED, Index.UUID).stream()
        .filter(index::has)
  ...
  public void removeColumn(Index column) {
    table.removeColumn(
        table.getColumnModel().getColumn(index.get(column) - deletedColumnIndex.getAndIncrement()));
  }
