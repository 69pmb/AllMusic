...
/**
 * Décrit la façon dont les tableaux contenant des {@code Fichier} seront affichées pour l'onglet
 * {@code FichierPanel}.
 *
...
public class FichierPanelModel extends AbstractModel {
  ...
  public Class<?> getColumnClass(int col) {
    Class<?> result;
    if (Arrays.asList(
            FichierPanel.getFichierindex().get(Index.PUBLISH),
            FichierPanel.getFichierindex().get(Index.FILE_SIZE),
            FichierPanel.getFichierindex().get(Index.LINE_NUMBER))
        .contains(col)) {
      result = Integer.class;
    } else if (col == FichierPanel.getFichierindex().get(Index.CREATE_DATE)) {
      result = LocalDateTime.class;
    } else if (Arrays.asList(
            FichierPanel.getFichierindex().get(Index.PERCENT_DELETED),
            FichierPanel.getFichierindex().get(Index.SCORE),
            FichierPanel.getFichierindex().get(Index.SCORE_DELETED))
        .contains(col)) {
      result = Double.class;
    } else {
      result = String.class;
