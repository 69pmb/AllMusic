...
public class CompoFichierPanelModel extends AbstractModel {
  ...
  public Class<?> getColumnClass(int col) {
    Class<?> result;
    if (Arrays.asList(
            FichierPanel.getCompositionindex().get(Index.RANK),
            FichierPanel.getCompositionindex().get(Index.FILE_SIZE),
            FichierPanel.getCompositionindex().get(Index.LINE_NUMBER))
        .contains(col)) {
