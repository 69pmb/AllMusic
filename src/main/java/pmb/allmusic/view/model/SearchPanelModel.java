...
import pmb.allmusic.view.panel.SearchPanel;

/**
 * Décrit la façon dont les tableaux contenant des {@code Composition} seront affichées pour
 * l'onglet {@code SearchPanel}.
 *
...
public class SearchPanelModel extends AbstractModel {
  ...
  public Class<?> getColumnClass(int col) {
    Class<?> result;
    if (Arrays.asList(
            SearchPanel.getIndex().get(Index.FILE_SIZE),
            SearchPanel.getIndex().get(Index.SCORE),
            SearchPanel.getIndex().get(Index.LINE_NUMBER),
            SearchPanel.getIndex().get(Index.DECILE))
        .contains(col)) {
      ...
      if (BooleanUtils.isTrue(
          (Boolean)
              ((Vector<?>) dataVector.elementAt(i))
                  .elementAt(SearchPanel.getIndex().get(Index.SELECTED)))) {
