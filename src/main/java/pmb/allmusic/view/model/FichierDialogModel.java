...
/**
 * Décrit la façon dont les tableaux contenant des {@code Fichier} seront affichées pour {@code
 * DialogFileTable}.
 *
...
public class FichierDialogModel extends AbstractModel {
  ...
  public Class<?> getColumnClass(int col) {
    Class<?> result;
    if (Arrays.asList(
            DialogFileTable.getIndex().get(Index.PUBLISH),
            DialogFileTable.getIndex().get(Index.FILE_SIZE),
            DialogFileTable.getIndex().get(Index.RANK))
        .contains(col)) {
