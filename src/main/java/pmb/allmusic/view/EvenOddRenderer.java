...
public class EvenOddRenderer extends DefaultTableCellRenderer {
  ...
  private static final Color[] DECILE_SCORE_PURPLE = {
    new Color(243, 233, 252),
    new Color(219, 191, 246),
    new Color(196, 149, 240),
    new Color(173, 106, 234),
    new Color(149, 64, 228),
    new Color(138, 43, 226),
    new Color(110, 34, 180),
    new Color(82, 25, 135),
    new Color(55, 17, 90),
    new Color(27, 8, 45)
  };
  ...
  public Component getTableCellRendererComponent(
    ...
    if (index.has(Index.DELETED)) {
      ...
      rowDeleted =
          Boolean.valueOf((String) getValueByColumnIndex(table, row, index.get(Index.DELETED)));
    }
    ...
    if (index.has(Index.TYPE) && column == index.get(Index.TYPE)) {
      ...
      background = getDefaultBackground(isSelected, row, rowDeleted);
    } else if (index.has(Index.SCORE)
        && index.has(Index.DECILE)
        && column == index.get(Index.SCORE)) {
      return getScoreRenderer(table, isSelected, row, renderer, font);
    } else if (index.has(Index.SORTED)
        && (column == index.get(Index.SORTED) - 1
            || column == index.get(Index.SORTED)
            || (index.has(Index.RANK) && column == index.get(Index.RANK)))) {
  ...
  private Color getTypeRenderer(
    ...
    RecordType type =
        RecordType.getByValue((String) getValueByColumnIndex(table, row, index.get(Index.TYPE)));
  ...
  private Component getScoreRenderer(
      JTable table, boolean isSelected, int row, Component renderer, Font font) {
  ...
  private Component getSortRenderer(JTable table, int row, Component renderer) {
    ...
    Boolean sorted =
        StringUtils.equalsIgnoreCase(sortedString, "oui") ? Boolean.TRUE : Boolean.FALSE;
  ...
  private void setTooltip(JTable table, Object value, int row, int column) {
    ...
    } else if (index.has(Index.DECILE)
        && index.has(Index.SCORE)
        && column == index.get(Index.SCORE)) {
      // if decile
      setToolTipText(
          String.valueOf(getValueByColumnIndex(table, row, index.get(Index.DECILE))) + " / 10");
    } else {
      setToolTipText(null);
    }
  }
  ...
  private static Object getValueByColumnIndex(JTable table, int row, int index) {
    return ((Vector<Object>)
            ((AbstractModel) table.getModel())
                .getDataVector()
                .get(table.getRowSorter().convertRowIndexToModel(row)))
        .get(index);
  }
}
