 ...
 * <ul>
 *   Batchs List :
 *   <li>Find duplicate compositions
 *   <li>Finds and edits composition with a slash in title and alone: slashEdit
 *   <li>Checks if deleted for given a list of compositions
 *   <li>Mass deletion: for deleting an important amount of compositions
 * </ul>
 */
public class BatchEditPanel {
  ...
  private void findDuplicateComposition() {
    ...
    // Launch Button
    JButton fdcBtn =
        ComponentBuilder.buildJButton("Go Compositions En Double", 200, Constant.ICON_GO);
    fdcBtn.setToolTipText(
        "Fusionne les compositions identiques mais non détectées à la fusion classique.");
    fdcBtn.addActionListener(
        (ActionEvent arg0) -> {
          LOG.debug("Start findDuplicateComposition");
          batchPanel.displayText(
              "Start findDuplicateComposition: " + VariousUtils.getCurrentTime(), false);
          new Thread(
                  () -> {
                    OngletPanel.getArtist().interruptUpdateArtist(true);
                    batchPanel.setFileResult(
                        BatchEditUtils.detectsDuplicateFinal(
                            fdcSong.isSelected(),
                            fdcAlbum.isSelected(),
                    ...
                    batchPanel.displayText(
                        "End findDuplicateComposition: " + VariousUtils.getCurrentTime(), false);
  ...
  private void slashEdit() {
    ...
    // Slash Btn
    JButton slashBtn =
        ComponentBuilder.buildJButton("Go Compositions Slash", 200, Constant.ICON_GO);
    slashBtn.setToolTipText("Editer des compositions seules contenant un slash dans leur titre");
    slashBtn.addActionListener(
        (ActionEvent arg0) -> {
          ...
          new Thread(
                  () -> {
                    ...
                    batchPanel.displayText(
                        "End slashEdit: " + VariousUtils.getCurrentTime(), false);
                  })
              .start();
        });
  ...
  private void checksIfDeleted() {
    ...
    // File or directory
    JCheckBox isDirectory =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(checks)
            .withLabel("Sélectionner un dossier : ")
            .build();

    // File chooser
    JLabel selectedFile = new JLabel();
    FileFilter fileFilter = new FileNameExtensionFilter("txt", "txt");
    BatchPanel.buildFileChooser(
            checks, "Charge un fichier txt contenant des chansons", selectedFile, fileFilter)
        .whenCompleteAsync(
            (chooser, err) ->
                checksIfDeleted(checks, isDirectory, selectedFile, fileFilter, chooser));

    batchPanel.getRoot().add(checks);
    LOG.debug("End checksIfDeleted");
  }

  public void checksIfDeleted(
      JPanel checks,
      JCheckBox isDirectory,
      JLabel selectedFile,
    ...
    // Bouton d'action
    JButton checksBtn =
        ComponentBuilder.buildJButton("Go Checks If Deleted", 200, Constant.ICON_GO);
    checksBtn.setToolTipText("Vérifie dans le fichier donné si les compositions sont supprimées");
    checksBtn.addActionListener(
        (ActionEvent arg0) -> {
          if (selectedFile.getText() != null
              && !StringUtils.equalsIgnoreCase(
                  selectedFile.getText(), Constant.getResourcesDir())) {
            LOG.debug("End browse");
            batchPanel.displayText(
                "Start checksIfDeleted: " + VariousUtils.getCurrentTime(), false);
            new Thread(
                    () -> {
                      BatchEditUtils.checksIfDeleted(
                          new File(selectedFile.getText()), (RecordType) type.getSelectedItem());
                      batchPanel.displayText(
                          "End checksIfDeleted: " + VariousUtils.getCurrentTime(), false);
                    })
                .start();
          } else {
  ...
  /**
   * Mass deletion: import a file of compositions to delete, by type. A search and a confirmation is
   * done for each.
   */
  private void massDeletion() {
    ...
    BatchPanel.buildFileChooser(
        massDeletion,
        "Charge un fichier csv contenant des compositions",
        selectedFile,
    ...
    // Bouton d'action
    JButton massDeletionBtn =
        ComponentBuilder.buildJButton("Go Mass Deletion", 200, Constant.ICON_GO);
    massDeletionBtn.setToolTipText("Supprime en masse des compositions.");
    massDeletionBtn.addActionListener(
        (ActionEvent arg0) -> {
          if (selectedFile.getText() != null
              && !StringUtils.equalsIgnoreCase(
                  selectedFile.getText(), Constant.getResourcesDir())) {
            ...
            new Thread(
                    () -> {
                      batchPanel.setFileResult(
                          BatchEditUtils.massDeletion(
                              type.getSelectedItem().toString(), new File(selectedFile.getText())));
                      batchPanel.displayText(
                          "End massDeletion: " + VariousUtils.getCurrentTime(), false);
                    })
                .start();
          } else {
            batchPanel.displayText("No selected file", false);
  ...
  private static JComboBox<RecordType> buildTypeComboBox(JPanel parent) {
    ...
    JComboBox<RecordType> type =
        new JComboBox<>(new RecordType[] {RecordType.SONG, RecordType.ALBUM});
