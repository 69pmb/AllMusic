...
public class SearchPanel extends JPanel implements ModificationComposition, ActionPanel {
  ...
  private static final String[] title = {
    "#", "Artiste", "Titre", "Type", "Nombre de fichiers", "Score", "", "", "", ""
  };
  ...
  private void initButtons(JPanel header) {
    ...
    // Clear Btn
    JButton clear =
        ComponentBuilder.buildJButton("Réinitialiser recherche", 200, Constant.ICON_ERASE);
    ...
    JButton delete =
        ComponentBuilder.buildJButton(
            "Supprimer les compositions sélectionnées", 300, Constant.ICON_DELETE);
                    ...
                    .map(
                        v ->
                            VariousUtils.stringToUuids(
                                    ((Vector<String>) v)
                                        .get(SearchPanel.getIndex().get(Index.UUID)))
                                .get(0))
    ...
    JButton modif =
        ComponentBuilder.buildJButton(
            "Modifier la composition sélectionnée", 300, Constant.ICON_EDIT);
    modif.addActionListener(
        (ActionEvent e) -> {
          try {
            ...
            ExceptionDialog ed =
                new ExceptionDialog(
                    "An exception has been thrown when editing composition", e2.getMessage(), e2);
            ed.setVisible(true);
          }
        });
    top.add(modif);
    ...
    JButton csv =
        ComponentBuilder.buildJButton(
            "Télécharger le résultat de la recherche en CSV", 300, Constant.ICON_DOWNLOAD);
  ...
  private void generatesCsvFile() {
    ...
    csvHeader.add(
        "Critères: "
            + Arrays.asList(
                    ...
                    fileName.getText(),
                    Optional.ofNullable(searchMethod.getSelectedItem())
                        .map(Object::toString)
                        .orElse(""),
    ...
    String name =
        CsvFile.exportCsv(
            "search",
            PanelUtils.convertDataVectorToList(tableResult.getTable()),
            null,
  ...
  private void initSearchFields(JPanel header) {
    ...
    artist =
        new ComponentBuilder<MyInputText, String>(MyInputText.class)
            .withParent(topFields)
            .withAsyncValues(OngletPanel::getArtistList, OngletPanel.getAsyncList())
            .withLabel("Artiste : ")
            .withResize(topResize)
            .build();
    // Titre
    titre =
        new ComponentBuilder<MyInputText, String>(MyInputText.class)
            .withParent(topFields)
            .withAsyncValues(OngletPanel::getTitleList, OngletPanel.getAsyncList())
            .withLabel("Titre : ")
            .withResize(topResize)
            .build();
    // SearchMethod
    ...
    range =
        new ComponentBuilder<MyInputRange, String>(MyInputRange.class)
            ...
            .withResize(topResize.addPanelWidth(50))
            .build();
    // Nom du fichier
    fileName =
        new ComponentBuilder<MyInputText, String>(MyInputText.class)
    ...
    inFiles =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(bottomFields)
            .withLabel("<html>Rechercher dans les fichiers : </html>")
  ...
  private void initTable() {
    ...
    try {
      tableResult =
          new TableBuilder()
              .withModelAndData(null, title, SearchPanelModel.class)
              ...
              .withMouseClickedActions(
                  ...
                  true)
              .withPopupMenu(
                  new CompositionPopupMenu(this.getClass(), null, SearchPanel.getIndex()))
              .withKeyListener()
              .build();
    } catch (MajorException e1) {
    ...
    if (!allCompo.isEmpty()) {
      ...
      compoResult.addAll(
          SearchUtils.search(
              ...
              criteria,
              inFiles.isSelected(),
              SearchMethod.getByValue((String) searchMethod.getSelectedItem()),
              deleted.isSelected(),
              true));
      updateTable(true);
    }
  ...
  public void updateTable(boolean scrollTop) {
    ...
    tableResult.updateTable(
        CompositionUtils.convertCompositionListToVector(
            compoResult, null, false, true, true, true, true),
        new SortKey(index.get(Index.SCORE), SortOrder.DESCENDING),
        scrollTop);
  ...
  public void modifyCompositionAction(Vector<String> selectedRow) throws MajorException {
    if (tableResult.getModel().getSelected().size() > 1) {
      ...
      compoResult =
          PanelUtils.editCompositionAction(selectedRow, compoResult, SearchPanel.getIndex());
      updateTable(false);
    }
  }
  ...
  public void splitCompositionAction(Vector<Object> selected) {
    ...
    if (tableResult.getModel().getSelected().size() > 1) {
      ...
      compoResult =
          PanelUtils.splitCompositionAction(selected, compoResult, SearchPanel.getIndex());
      updateTable(false);
    }
