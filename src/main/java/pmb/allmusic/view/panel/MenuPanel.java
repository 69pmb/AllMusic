...
public final class MenuPanel {
  ...
  private static JMenu fichierMenu() {
    ...
    ComponentBuilder.buildMenuItem(
        ...
        KeyEvent.VK_L,
        (ActionEvent ae) -> openFile(MyConstant.FILE_LOG_PATH, "Error when opening log file"),
        null);
    ...
    ComponentBuilder.buildMenuItem(
        fichier,
        "Ouvrir le fichier de modification",
        KeyEvent.VK_D,
        (ActionEvent ae) -> openFile(Constant.MODIF_FILE_PATH, "Error when opening modif file"),
        null);
    ...
    ComponentBuilder.buildMenuItem(
        ...
        KeyEvent.VK_O,
        (ActionEvent ae) ->
            openDirectory(Constant.getOutputDir(), "Error when opening output directory"),
        null);

    ComponentBuilder.buildMenuItem(
        fichier,
        "Ouvrir le dossier des fichiers XML",
        KeyEvent.VK_L,
        (ActionEvent ae) ->
            openDirectory(Constant.getXmlPath(), "Error when opening XML files directory"),
        null);

    ComponentBuilder.buildMenuItem(
        fichier,
        "Ouvrir le dossier Musique",
        KeyEvent.VK_M,
        (ActionEvent ae) ->
            openDirectory(
                Constant.getMusicAbsDirectory(), "Error when opening txt files directory"),
        null);
  ...
  private static JMenu edititonMenu() {
                    ...
                    () -> {
                      try {
                        ...
                        JOptionPane.showMessageDialog(
                            null,
                            "Final File successfully exported.",
                            "",
                        ...
                        ExceptionDialog exceptionDialog =
                            new ExceptionDialog(
                                "Export of final file for Angular failed", e.getMessage(), e);
    ...
    ComponentBuilder.buildMenuItem(
        ...
        (ActionEvent ae) -> {
          if (MyProperties.reload()) {
            JOptionPane.showMessageDialog(
                null, "Properties successfully reloaded.", "", JOptionPane.INFORMATION_MESSAGE);
          } else {
            JOptionPane.showMessageDialog(
                null, "Error when reloading properties.", "", JOptionPane.ERROR_MESSAGE);
          }
        },
        ActionEvent.CTRL_MASK + InputEvent.SHIFT_DOWN_MASK);
    ...
    ComponentBuilder.buildMenuItem(
        edition,
        "Recalculer les données de l'onglet Artiste",
        KeyEvent.VK_E,
        (ActionEvent ae) -> {
        ...
        .forEach(
            laf -> {
              ...
              mi.addActionListener(
                  e ->
                      Arrays.stream(UIManager.getInstalledLookAndFeels())
                          .filter(look -> laf.getName().equals(look.getName()))
                          .findFirst()
                          .map(LookAndFeelInfo::getClassName)
                          .ifPresent(
                              look -> {
                                if (!PanelUtils.setLookAndFeel(look)) {
                                  PanelUtils.setLookAndFeel(
