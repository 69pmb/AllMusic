...
public class ImportPanel extends JPanel implements ActionPanel {
  ...
  private void insertFirstLine() {
    ...
    name =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            .withParent(firstLine)
            .withLabel("Nom du fichier : ")
            .withPanelWidth(360)
            .withComponentWidth(350)
            .build();
        ...
        new FocusListener() {
          ...
          public void focusLost(FocusEvent e) {
            ...
            if (new File(absolutePathFileXml).exists()) {
              miseEnFormeResultLabel(
                  new LinkedList<>(Arrays.asList(name.getText() + " existe déjà")));
            }
          }
    ...
    author =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            ...
            .withLabel("Auteur : ")
            .withPanelWidth(160)
            .withComponentWidth(150)
            .build();
    // Date de creation
    date =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            .withParent(firstLine)
            .withLabel("Date de création : ")
            .withPanelWidth(160)
            .withComponentWidth(150)
            .build();
    ...
    // Type
    type =
        new ComponentBuilder<JComboBox, RecordType>(JComboBox.class)
            .withParent(firstLine)
            .withPanelWidth(110)
            .withLabel("Type : ")
            .withValues(RecordType.values())
            .withComponentWidth(100)
            .build();
    // Categorie
    cat =
    ...
    sorted =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            ...
            .withLabel("Classé : ")
            .withPanelWidth(60)
            .withComponentWidth(25)
            .build();
    // Order/Artist
    order =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(firstLine)
            .withLabel("Artiste en premier : ")
  ...
  private void insertSecondLine() {
    ...
    separator =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            .withParent(secondLine)
            .withLabel("<html>Séparateur trouvé : </html>")
            .withPanelWidth(120)
            .withComponentWidth(20)
            .build();
  ...
  private void insertThirdLine() {
    ...
    reverseArtist =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(thirdLine)
            .withLabel("<html>Retourner l'artiste: </html>")
            .withPanelWidth(100)
            .withComponentWidth(20)
            .build();
    // removeParenthese
    removeParenthese =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(thirdLine)
            .withLabel("<html>Supprimer le texte entre parenthèse du titre: </html>")
            .withPanelWidth(150)
            .withComponentWidth(20)
            .build();
    // upper
    upper =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(thirdLine)
            .withLabel("<html>Pas de séparateur, artiste en capitale: </html>")
            .withPanelWidth(100)
            .withComponentWidth(20)
            .build();
    // removeAfter
    removeAfter =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(thirdLine)
            .withLabel("<html>Supprime après le dernier séparateur: </html>")
            .withPanelWidth(100)
            .withComponentWidth(20)
            .build();
    // isCompleteDirectory
    isCompleteDirectory =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(thirdLine)
            .withLabel("<html>Utiliser le dossier du fichier pour la mise en forme: </html>")
            .withPanelWidth(150)
            .withComponentWidth(20)
            .build();
    ...
    characterToRemove =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            .withParent(cleanBtnPanel)
            .withLabel("<html>Caractères à supprimer : </html>")
            .withPanelWidth(140)
            .withComponentWidth(40)
            .build();
    ...
    maxLengthClean =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            .withParent(cleanBtnPanel)
            .withLabel(
                "<html><body style='width: 100%'>Longueur maximale d'une ligne valide: </body></html>")
            .withInitialValue("120")
            .withPanelWidth(120)
            .withComponentWidth(40)
            .build();
    ...
    isBefore =
        new ComponentBuilder<JCheckBox, Boolean>(JCheckBox.class)
            .withParent(cleanBtnPanel)
            .withLabel("<html>Supprimer au début : </html>")
            .withPanelWidth(120)
            .withInitialValue(true)
            .withComponentWidth(20)
            .build();
  ...
  private void insertBottomPanel() {
    ...
    importFile.setToolTipText(
        "Importe au format XML le fichier chargé précédemment avec les critères renseignés.");
    importFile.addActionListener((ActionEvent arg0) -> importFileAction());
    bottom.add(importFile);

    // Clean
    JButton cleanFile =
        ComponentBuilder.buildJButton("Nettoyer le fichier", 200, Constant.ICON_CLEAN);
    ...
    JButton mef =
        ComponentBuilder.buildJButton(
            "Mettre en forme un fichier ou dossier", 250, Constant.ICON_ALIGN);
    mef.setToolTipText("Pour supprimer les diacritiques et remplacer des charactères spéciaux.");
    mef.addActionListener(
        (ActionEvent arg0) -> {
          result =
              new LinkedList<>(
                  Arrays.asList(
                      (isCompleteDirectory.isSelected() ? "Dossier" : "Fichier")
                          + " mis en forme:"));
    ...
    JButton openFile =
        ComponentBuilder.buildJButton("Ouvrir le fichier source", 200, Constant.ICON_TXT_FILE);
    ...
    generated =
        new ComponentBuilder<JComboBox, String>(JComboBox.class)
            .withParent(wrapGenerated)
            .withPanelWidth(200)
            .withComponentWidth(100)
            .withValues(new String[] {GENERATED_XML, GENERATED_CLEAN, GENERATED_MISE_EN_FORME})
            .withInitialValue(GENERATED_XML)
            .build();
    generated.addActionListener(
        (ActionEvent arg0) -> selectGeneratedFile((String) generated.getSelectedItem()));
    wrapGenerated.add(generated);

    openGenerated =
        ComponentBuilder.buildJButton("Ouvrir le fichier généré", 200, Constant.ICON_XML_FILE);
    openGenerated.setToolTipText("Ouvre le fichier généré seléctionné dans Notepad++");
    openGenerated.addActionListener(
        (ActionEvent arg0) ->
            getGeneratedFile((String) generated.getSelectedItem())
    ...
    // Fusion
    JButton fusionFile =
        ComponentBuilder.buildJButton("Fusionner tous les fichiers", 200, Constant.ICON_FUSION);
      ...
      try {
        ...
        List<Composition> compoList =
            ImportFile.getCompositionsFromFile(
                new File(absolutePathFileTxt),
                fichier,
                (RecordType) type.getSelectedItem(),
                separator.getText(),
                result,
                order.isSelected(),
                reverseArtist.isSelected(),
                removeParenthese.isSelected(),
                upper.isSelected(),
  ...
  private static List<Composition> mergeWithFormerData(
      List<Composition> compoList, String fileName) {
    List<Composition> oldList =
        FilesUtils.buildXmlFilePath(fileName)
            .map(ImportXML::importXML)
            .orElseThrow(
                () ->
                    new MinorException(
    ...
    return compoList.stream()
        .map(
            c ->
                findByRankAndFileName(oldList, c.getFiles().get(0).getClassement(), fileName)
                    .map(
                        old -> {
                    ...
                    .orElseThrow(() -> new MinorException("Can't find composition: " + c)))
        .collect(Collectors.toList());
  }

  private static Optional<Composition> findByRankAndFileName(
      List<Composition> list, Integer rank, String filename) {
    return SearchUtils.findBy(
        list,
        c ->
            Integer.compare(c.getFiles().get(0).getClassement(), rank) == 0
                && StringUtils.equals(filename, c.getFiles().get(0).getFileName()));
  }

  private Map<String, String> convertParamsToMap(
      String separator,
      boolean artistFirst,
  ...
  /**
   * Crée un file chooser pour sélectionner un fichier selon l'extension donnée et à l'endroit
   * donnée.
   *
  ...
  private File addBrowsingFile(String extension, String dir, ActionEvent e) {
    ...
    Optional.ofNullable(jfile.getActionMap().get("viewTypeDetails"))
        .ifPresent(
            a -> a.actionPerformed(new ActionEvent(e.getSource(), e.getID(), "viewTypeDetails")));
  ...
  /**
   * Traitement lorsqu'on séléctionne un fichier. Les paramètres d'import sont déduit du nom du
   * fichier et du contenu.
   */
  private void loadFile() {
    ...
    explorePath =
        StringUtils.substring(
            file.getAbsolutePath(), 0, file.getAbsolutePath().lastIndexOf(File.separator));
  ...
  private void initInputsWithImportParams(
      List<String> label, String firstLine, DateTimeFormatter fullDTF) {
    Map<String, String> value = new HashMap<>();
    try {
      value =
          MiscUtils.<String>readValueAsMap(
              StringUtils.substringAfter(firstLine, Constant.IMPORT_PARAMS_PREFIX));
    } catch (IOException e) {
      LOG.error(
          "Error while decoding import params: {} in file {}", firstLine, absolutePathFileTxt, e);
      return;
    }
    Optional.of(
        ...
        .ifPresent(crit -> LOG.debug("value: {}", crit));
    LOG.debug("Init with stored params");
    name.setText(value.get(IMPORT_PARAM_NAME));
    absolutePathFileXml =
        Constant.getXmlPath() + value.get(IMPORT_PARAM_NAME) + MyConstant.XML_EXTENSION;
  ...
  private void initInputsWithGuessedParams(
      List<String> label, List<String> randomLineAndLastLines, DateTimeFormatter fullDTF) {
    ...
    if (randomLineAndLastLines.size() == 6) {
      ...
      fichier.setSize(
          ImportFile.determineSize(fichier, randomLineAndLastLines, file.getAbsolutePath()));
    ...
    if (Cat.MISCELLANEOUS == fichier.getCategorie()
        && RecordType.UNKNOWN != determineType
        && fichier.getPublishYear() != 0
        && rangeDatesZero) {
  ...
  private Optional<String> getGeneratedFile(String type) {
    ...
    switch (type) {
      case GENERATED_CLEAN:
        generatedFile =
            CleanFile.buildGeneratedFilePath(new File(absolutePathFileTxt), CleanFile.SUFFIX_CLEAR);
        break;
      case GENERATED_XML:
      ...
      case GENERATED_MISE_EN_FORME:
        generatedFile =
            CleanFile.buildGeneratedFilePath(new File(absolutePathFileTxt), CleanFile.SUFFIX_MEF);
        break;
      default:
        generatedFile = null;
        break;
