 ...
 *   Batchs List :
 *   <li>Top year
 *   <li>Calculates in a file for each compositions, the number of files associates to it and get
 *       the average for the file: averageOfFilesByFiles
 *   <li>Looks for weird file size: weirdFileSize
 *   <li>By looking in other files, guess the type of unknown compositions: findUnknown
 *   <li>Generates general stats
 * </ul>
 */
public class BatchGeneratePanel {
  ...
  private void topYear() {
    ...
    topBtn.addActionListener(
        (ActionEvent arg0) -> {
          batchPanel.displayText("Start topYear: " + VariousUtils.getCurrentTime(), false);
          new Thread(
                  () -> {
                    batchPanel.setFileResult(
                        BatchGenerateUtils.topYear(
                            Integer.parseInt(yearBeginTop.getText()),
                            Integer.parseInt(yearEndTop.getText()),
                            Integer.parseInt(albumLimit.getText()),
  ...
  private void averageOfFilesByFiles() {
    ...
    averageBtn.setToolTipText(
        "Calcule la moyenne du nombre de fichier de chaque composition par fichier.");
    averageBtn.addActionListener(
        (ActionEvent arg0) -> {
  ...
  private void weirdFileSize() {
    ...
    // weird btn
    JButton weirdBtn =
        ComponentBuilder.buildJButton("Go Weird Size By File", 200, Constant.ICON_GO);
    weirdBtn.setToolTipText("Recherche les fichiers dont la taille est bizarre");
    weirdBtn.addActionListener(
        (ActionEvent arg0) -> {
          ...
          new Thread(
                  () -> {
                    ...
                    batchPanel.displayText(
                        "End findUnknown: " + VariousUtils.getCurrentTime(), false);
                  })
              .start();
        });
        ...
        (ActionEvent arg0) -> {
          ...
          new Thread(
                  () -> {
                    ...
                    batchPanel.displayText(
                        "End statistic: " + VariousUtils.getCurrentTime(), false);
                  })
              .start();
        });
