...
public final class PanelUtils {
  ...
  private static final BigDecimal widthBG =
      new BigDecimal(Toolkit.getDefaultToolkit().getScreenSize().width);
  private static final BigDecimal heightBG =
      new BigDecimal(Toolkit.getDefaultToolkit().getScreenSize().height);

  public static final FocusListener selectAll =
      new FocusListener() {
        @Override
  ...
  public static void colRenderer(JTable table, boolean lastColumn, ColumnIndex index) {
    ...
    setColumnsWidth(
        table, topComponent != null ? topComponent.getWidth() : table.getWidth(), "Init");
    ...
    for (int i = 0; i < columnCount; i++) {
      ...
      IntSummaryStatistics stats =
          IntStream.range(0, table.getRowCount())
              .mapToObj(j -> table.getValueAt(j, columnIndex))
              .filter(Objects::nonNull)
              .map(value -> String.valueOf(value).length())
      ...
      if (maxByColumn != 0) {
        ...
        columnModel.getColumn(columnIndex).setPreferredWidth(maxByColumn * 7); // valeur arbitraire
        columnWidth = Double.valueOf(maxByColumn) * 7D;
      }
      ...
      if (isTableWiderThanScreen) {
        ...
        colWidth.put(columnIndex, round);
      }
    }
    if (isTableWiderThanScreen) {
      ...
      if (sum.compareTo(width) > 0) {
        ...
        colWidth.entrySet().stream()
            .forEach(e -> e.setValue(e.getValue() - (int) Math.round(offset)));
      } else {
        LOG.trace("Too thin");
      ...
      colWidth.entrySet().stream()
          .forEach(e -> columnModel.getColumn(e.getKey()).setMaxWidth(e.getValue()));
      colWidth.entrySet().stream()
          .forEach(e -> columnModel.getColumn(e.getKey()).setMinWidth(e.getValue()));
    }
  ...
  public static Optional<Vector<String>> getSelectedRowByPoint(JTable target, Point point) {
    return getSelectedRow(
        target, target.rowAtPoint(SwingUtilities.convertPoint(target, point, target)));
  }
  ...
  public static <T extends AbstractModel> Optional<Vector<String>> getSelectedRow(
      JTable target, int rowAtPoint) {
  ...
  public static <T extends AbstractModel> List<List<String>> convertDataVectorToList(JTable table) {
    ...
    for (int i = 0; i < model.getRowCount(); i++) {
      Vector<Object> selectedRow =
          ((T) model).getDataVector().get(table.getRowSorter().convertRowIndexToModel(i));
      List<String> row = new ArrayList<>();
      for (int j = 0; j < model.getColumnCount(); j++) {
        if (model.getColumnClass(j) == Boolean.class) {
          row.add(String.valueOf(selectedRow.get(j)));
        } else if (model.getColumnClass(j) == Integer.class
            || model.getColumnClass(j) == Double.class
   ...
   * @param sortedColumn the sorted column (if artist go to first artist starting to the letter
   *     pressed)
   ...
   */
  public static void deleteCompositionAction(List<Composition> compoList, List<String> uuids)
    ...
    for (String uuid : uuids) {
      Composition toRemoveToFinal =
          CompositionUtils.findByUuid(importXML, Arrays.asList(uuid))
              .orElseThrow(
                  () -> new MajorException("Can't find composition to delete in final file"));
      // Update JTable Data
      CompositionUtils.findByUuid(compoList, Arrays.asList(uuid))
      ...
      try {
        for (Fichier file : toRemoveToFinal.getFiles()) {
          ...
          CompositionUtils.findByUuid(compoListFromData, toRemoveToFinal.getUuids())
              .ifPresent(c -> c.setDeleted(true));
        }
      } catch (Exception e) {
   ...
   * @throws MajorException if something went wrong
   */
  public static List<Composition> editCompositionAction(
      Vector<String> selectedRow, List<Composition> compositionList, ColumnIndex index)
      throws MajorException {
    ...
    Composition toModif =
        CompositionUtils.findByUuid(importXML, Arrays.asList(uuid))
            .orElseThrow(
                () ->
                    new MajorException(
                        "Can't find composition: " + selectedRow + " to edit in final file"));
    ...
    importXML =
        importXML.stream().filter(c -> !c.getUuids().contains(uuid)).collect(Collectors.toList());
    ...
    if (!compoExist.isPresent()) {
      ...
      CompositionUtils.findByUuid(compositionList, Arrays.asList(uuid))
          .ifPresent(
              c -> {
      ...
      compositionList =
          compositionList.stream()
              .filter(c -> !c.getUuids().contains(uuid))
    ...
    try {
      CompositionUtils.editCompositionsInFiles(
          toModif, compoExist.map(Composition::isDeleted).orElse(toModif.isDeleted()));
    } catch (MajorException e1) {
   ...
   *
   * <p>Recover composition's files by name and find composition edited by uuid. And then replace it
   * by the given composition.
   *
  ...
  private static void updateFichierPanelData(Composition edited) {
    for (Fichier file : edited.getFiles()) {
      CompositionUtils.findByUuid(
              OngletPanel.getFichier().getCompoListFromData(file).stream()
                  .filter(Objects::nonNull)
                  .collect(Collectors.toList()),
              edited.getUuids())
   ...
   */
  public static List<Composition> splitCompositionAction(
      Vector<Object> selected, List<Composition> compositionList, ColumnIndex index) {
    LOG.debug("Start splitCompositionAction");
    OngletPanel.getArtist().interruptUpdateArtist(true);
    // Lancement de la popup de modification
    SplitCompositionDialog sd =
        new SplitCompositionDialog(
            (String) selected.get(index.get(Index.TITLE)),
            selected.get(index.get(Index.ARTIST)) + " - " + selected.get(index.get(Index.TYPE)));
    sd.show();
    String t1;
    String t2;
    ...
    edited.getFiles().stream()
        .forEach(
            file -> {
              String newUuid = VariousUtils.getUuid();
              newUuids.add(newUuid);
              List<Composition> xmlFile =
                  ImportXML.importXML(FilesUtils.buildXmlFilePath(file.getFileName()).orElse(""));
              xmlFile =
                  PanelUtils.splitComposition(
                      xmlFile, t1, t2, edited.getUuids(), Arrays.asList(newUuid), false);
              try {
    ...
    // Update given list
    compositionList =
        PanelUtils.splitComposition(compositionList, t1, t2, edited.getUuids(), newUuids, true);
    // Update fichier panel
    List<String> ids = ListUtils.union(newUuids, edited.getUuids());
    List<Fichier> filesToUpdate = new ArrayList<>();
    importXML.stream()
        .filter(c -> c.getUuids().stream().anyMatch(ids::contains))
        .forEach(c -> filesToUpdate.addAll(c.getFiles()));
   ...
   * @return the list updated
   */
  public static List<Composition> splitComposition(
    ...
    if (found.isPresent()) {
      ...
      if (fusion) {
        list =
            list.stream()
                .filter(c -> !c.getUuids().contains(uuids.get(0)))
                .collect(Collectors.toList());
   ...
   */
  public static void removeFichierFromComposition(
      List<Composition> list, List<String> uuids, Predicate<Fichier> filterFile)
      throws MajorException {
    Composition finalCompo =
        CompositionUtils.findByUuid(list, uuids)
            .orElseThrow(() -> new MajorException("Can't find composition in given list"));
    if (filterFile != null) {
      finalCompo.setFiles(
          finalCompo.getFiles().stream().filter(filterFile).collect(Collectors.toList()));
    }
    finalCompo.setUuids(
        finalCompo.getUuids().stream()
            .filter(ids -> !uuids.contains(ids))
            .collect(Collectors.toList()));
  }
  ...
  /**
   * Sets a given label wrapped text. Surrounds the text with an <{@code <html>} tag.
   *
  ...
  /**
   * Sets to the given panel a {@link FlowLayout} with a {@link FlowLayout#LEADING} alignment, 2
   * width gap and no height gap.
   *
   * @param panel to edit
   */
  ...
  public static void addComponent(
      JPanel panel, JComponent component, float alignement, int rigidSize) {
   ...
   */
  public static void setSizeByScreenSize(
      Component comp, double percentWidth, double percentHeight) {
  ...
  private static double getPercent(final BigDecimal bg, double percent) {
    return bg.divide(BigDecimal.valueOf(100D), RoundingMode.HALF_UP)
