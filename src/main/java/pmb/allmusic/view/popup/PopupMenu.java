...
/**
 * Parent class for creating a popup menu. Contains fields, init methods and builder for {@link
 * JMenuItem}.
 *
 * @see {@link JPopupMenu}
 * @author PBR
...
public class PopupMenu {
  ...
  public void buildCopyArtistAndTitleMenu(int shortcut, int artistIndex, int titleIndex) {
    ComponentBuilder.buildMenuItem(
        menu,
        "Copier l'artiste et le titre",
        shortcut,
        (ActionEvent e) -> {
          LOG.debug("Start copy A+T");
          VariousUtils.clipBoardAction(
              selectedRow.get(artistIndex) + " " + selectedRow.get(titleIndex));
          this.setVisible(false);
    ...
    ComponentBuilder.buildMenuItem(
        ...
        (ActionEvent e) -> {
          ...
          try {
            ...
            LOG.error(
                "Error when opening with notepad file : {}", selectedRow.get(fileNameIndex), e1);
          }
  ...
  public void buildOpenTxtFileMenu(int shortcut, ColumnIndex index) {
    ComponentBuilder.buildMenuItem(
        ...
        (ActionEvent e) -> {
          ...
          try {
            FilesUtils.openFileInNotepad(
                FilesUtils.buildTxtFilePath(
                        (String) selectedRow.get(index.get(Index.FILE_NAME)),
                        (String) selectedRow.get(index.get(Index.AUTHOR)))
                    .orElse(null),
                index.getOrDefault(Index.RANK, null));
            this.setVisible(false);
          } catch (MajorException e1) {
            LOG.error(
                "Error when opening with notepad file : {}",
                selectedRow.get(index.get(Index.FILE_NAME)),
                e1);
          }
