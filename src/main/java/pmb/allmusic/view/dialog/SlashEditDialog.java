...
/**
 * Dialog editing composition with a slash in title to split them into 2 compositions.
 *
 * @see {@link JDialog}
 */
public class SlashEditDialog extends AbstractDialog {
  ...
  public final FocusListener searchForMerge =
      new FocusListener() {
        @Override
  ...
  protected void initComposants() {
    ...
    title1 =
        new ComponentBuilder<JTextField, String>(JTextField.class)
            .withComponentWidth(500)
            .withPanelWidth(width)
            .withLabel("Titre 1")
            .withParent(compoPanel)
            .build();
    ...
    yes.addActionListener(
        (ActionEvent a) -> {
          if (StringUtils.isNotBlank(title1.getText())
              && StringUtils.isNotBlank(title2.getText())) {
            validate();
            finalFile = ImportXML.importXML(Constant.getFinalFilePath());
  ...
  private String findIfMergeable(Composition c, String title) {
    ...
    if (compoExist != null) {
      result =
          "Fusion possible avec "
              + compoExist.getArtist()
              + " - "
              + compoExist.getTitre()
              + " ("
              + compoExist.getFiles().size()
              + " fichier(s))";
    }
    return result;
  }
