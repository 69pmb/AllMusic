...
/**
 * Abstract dialog to configure filterable dialog diplaying a list of item with a checkbox in
 * header.
 *
...
public abstract class AbstractFilterDialog<T> extends AbstractDialog {
  ...
  public AbstractFilterDialog(
      ...
      List<T> data,
      Predicate<T> filter,
      Function<List<T>, Vector<Vector<Object>>> convertor,
      SortKey defaultSort,
      String label) {
  ...
  /**
   * Updates data, filters and converts it. If filtering is possible, a checkbox is added in
   * dialog's header.
   *
   * @param data
   */
