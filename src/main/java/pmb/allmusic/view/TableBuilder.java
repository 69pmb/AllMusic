...
public class TableBuilder {
  ...
  private static final Set<Index> UNSORTED_COLUMNS =
      Set.of(Index.LINE_NUMBER, Index.DECILE, Index.DELETED, Index.UUID);
  ...
  public TableBuilder withModelAndData(
      Vector<Vector<Object>> data, String[] header, Class<? extends AbstractModel> modelType)
    ...
    try {
      if (data == null) {
        ...
        model =
            modelType
                .getDeclaredConstructor(Vector.class, Vector.class)
                .newInstance(data, new Vector(Arrays.asList(header)));
      }
  ...
  /**
   * Initializes the row sorter. Columns defined in the constant {@link
   * TableBuilder#UNSORTED_COLUMNS} are not sorted.
   *
   * @return the table builder
   */
    ...
    TableRowSorter<TableModel> sorter =
        new TableRowSorter<>(table.getModel()) {
          ...
          public boolean isSortable(int column) {
            return UNSORTED_COLUMNS.stream()
                .filter(table.getIndex()::has)
  ...
  /**
   * Initializes the row sorter listener with selected column sortable on deleted composition.
   *
   * @return the table builder
   */
                ...
                if (e.getType() == RowSorterEvent.Type.SORT_ORDER_CHANGED) {
                  ...
                  if (column == table.getIndex().get(Index.SELECTED)) {
                    ...
                    table.setSortDeletedOrder(
                        table.getSortDeletedOrder() == SortOrder.ASCENDING
                        ...
                        new LinkedList<>(
                            Arrays.asList(
                                new RowSorter.SortKey(
                                    table.getSortedColumn(), table.getSortDeletedOrder())));
            ...
            new KeyListener() {
              ...
              public void keyReleased(KeyEvent e) {
                if (table.getPopupMenu() != null
                  ...
                  PanelUtils.getSelectedRow(table.getTable(), table.getTable().getSelectedRow())
                      .ifPresent(
                          row -> {
                            table.getPopupMenu().setSelectedRow(row);
                            Point pointAtRow =
                                PanelUtils.pointAtRow(
                                    table.getTable(), table.getTable().getSelectedRow());
                ...
                if (e.getKeyCode() == KeyEvent.VK_SHIFT && table.getPopupMenu() != null) {
                  ...
                  table.setSelectedRow(
                      PanelUtils.keyShortcutAction(
                          e, table.getSelectedRow(), table.getSortedColumn()));
                }
              }
  ...
  /**
   * Adds a mouse left click listener using the selected row. Adds another which shows the popup
   * menu when the mouse right button is clicked.
   *
   * @param leftClickAction the action executed when the mouse left button is clicked. 1st arg is
   *     the event, 2nd is the selected row
   * @param isDbClick if double click or not
   * @return the table builder
   */
  public TableBuilder withMouseClickedActions(
      BiConsumer<MouseEvent, Vector<String>> leftClickAction, boolean isDbClick) {
    withMouseClickedAction(
        e -> {
          Optional<Vector<String>> selectedRow =
          ...
          if (SwingUtilities.isRightMouseButton(e)) {
            table.getPopupMenu().show(e);
          } else if (e.getClickCount() == clickClount
