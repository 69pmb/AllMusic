...
/**
 * Classe pour exporter des fichiers au format XML pour être utilisé par une application Angular.
 */
public final class NgExportXml extends ExportXML {
    ...
    compList.forEach(
        composition ->
            exportFichier(
                ...
                fullDTF,
                formatComposition(
                    listComp.addElement(CompoHandler.TAG_COMPOSITION),
                    entrySet,
                    composition,
                    null)));
        ...
        .forEach(
            (key, value) -> {
              ...
              value.forEach(
                  comp ->
                      formatComposition(
                          fichier.addElement(CompoHandler.TAG_COMPOSITION), entrySet, comp, key));
            });
  ...
  private static String getType(List<Composition> list) {
    Map<RecordType, Long> collect =
        list.stream()
            .collect(Collectors.groupingBy(Composition::getRecordType, Collectors.counting()));
    if (collect.size() == 1) {
  ...
  private static Element formatComposition(
      Element comp, Set<Entry<String, String>> entrySet, Composition composition, Fichier fichier) {
        ...
        StringUtils.substringBefore(
            MiscUtils.removeParentheses(
                CleanFile.removeDiactriticals(
                    VariousUtils.cleanLine(composition.getArtist().toLowerCase(), entrySet))),
            " and ");
    ...
    comp.addAttribute(
        "s" + CompoHandler.TAG_TITRE,
        VariousUtils.removePunctuation(
            MiscUtils.removeParentheses(
                CleanFile.removeDiactriticals(
                    VariousUtils.cleanLine(composition.getTitre().toLowerCase(), entrySet)))));
    ...
    if (fichier != null) {
      composition.getFiles().stream()
          .filter(f -> StringUtils.equalsIgnoreCase(f.getFileName(), fichier.getFileName()))
          .findFirst()
  ...
  private static void saveFile(String fileName, Document doc) throws MajorException {
    ...
    try {
      ...
      throw new MinorException(
          "Error when deleting file: " + Constant.getXmlPath() + fullFileName, e);
    }
