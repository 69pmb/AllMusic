...
extends AbstractMappingStrategy<String, Integer, ComplexFieldMapEntry<String, Integer, T>, T> {
    ...
    /**
     * This method returns an empty array. The column position mapping strategy assumes that there is no header, and thus it also does not write one,
     * accordingly.
     *
    ...
    /**
     * Returns the column position for the given column number. Yes, they're the same thing. For this mapping strategy, it's a simple conversion from
    ...
    /**
     * Sets the {@link java.util.Comparator} to be used to sort columns when writing beans to a CSV file. Behavior of this method when used on a
     * mapping strategy intended for reading data from a CSV source is not defined.
     *
     * @param writeOrder The {@link java.util.Comparator} to use. May be {@code null}, in which case the natural ordering is used.
     * @since 4.3
     */
