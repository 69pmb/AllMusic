...
public class SearchPanel extends JPanel implements ModificationComposition {
    ...
    private void initTable() {
        ...
        try {
            tableResult = new TableBuilder().withModelAndData(null, title, CompoSearchPanelModel.class)
                    .withRowSorterListenerDelete(SearchPanel.getIndex().get(Index.LINE_NUMBER), SearchPanel.getIndex().get(Index.DELETED), SearchPanel.getIndex().get(Index.SELECTED))
    ...
    public void modifyCompositionAction(Vector<String> selectedRow) throws MyException {
        if (tableResult.getModel().getSelected().size() > 1) {
            ...
            compoResult = PanelUtils.editCompositionAction(selectedRow, compoResult, SearchPanel.getIndex().get(Index.ARTIST), SearchPanel.getIndex().get(Index.TITLE), SearchPanel.getIndex().get(Index.TYPE),
                    SearchPanel.getIndex().get(Index.DELETED), SearchPanel.getIndex().get(Index.UUID));
            updateTable();
        }
    }
