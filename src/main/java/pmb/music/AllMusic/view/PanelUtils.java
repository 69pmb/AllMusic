...
public final class PanelUtils {
    ...
    public static void setColumnsWidth(JTable table, int width, String name) {
        ...
        for (int i = 0; i < columnCount; i++) {
            ...
            IntSummaryStatistics stats = IntStream.range(0, table.getRowCount())
                    .mapToObj(j -> table.getValueAt(j, columnIndex)).filter(Objects::nonNull)
                    .map(value -> String.valueOf(value).length()).mapToInt(Integer::intValue).summaryStatistics();
    ...
    public static void deleteCompositionAction(List<Composition> compoList, List<String> uuids) throws MajorException {
        ...
        for (String uuid : uuids) {
            Composition toRemoveToFinal = CompositionUtils.findByUuid(importXML, Arrays.asList(uuid))
                    .orElseThrow(() -> new MajorException("Can't find composition to delete in final file"));
            // Update JTable Data
    ...
    public static List<Composition> editCompositionAction(Vector<String> selectedRow, List<Composition> compositionList,
        ...
        Composition toModif = CompositionUtils.findByUuid(importXML, Arrays.asList(uuid)).orElseThrow(
                () -> new MajorException("Can't find composition: " + selectedRow + " to edit in final file"));
        ...
        if (!compoExist.isPresent()) {
            ...
            compositionList = compositionList.stream().filter(c -> !c.getUuids().contains(uuid))
        ...
        try {
            CompositionUtils.editCompositionsInFiles(toModif,
                    compoExist.map(Composition::isDeleted).orElse(toModif.isDeleted()));
        } catch (MajorException e1) {
     ...
     * <p>
     * Recover composition's files by name and find composition edited by uuid. And
     * then replace it by the given composition.
     * </p>
     *
    ...
    private static void updateFichierPanelData(Composition edited) {
        for (Fichier file : edited.getFiles()) {
            CompositionUtils
            .findByUuid(OngletPanel.getFichier().getCompoListFromData(file).stream().filter(Objects::nonNull)
                    .collect(Collectors.toList()), edited.getUuids())
    ...
    public static List<Composition> splitCompositionAction(Vector<Object> selected, List<Composition> compositionList,
        ...
        OngletPanel.getArtist().interruptUpdateArtist(true);
        // Lancement de la popup de modification
        SplitCompositionDialog sd = new SplitCompositionDialog((String) selected.get(index.get(Index.TITLE)),
        ...
        importXML.stream().filter(c -> c.getUuids().stream().anyMatch(ids::contains))
     ...
     * @return the list updated
     */
    public static List<Composition> splitComposition(List<Composition> list, String title1, String title2,
            List<String> uuids, List<String> newUuids, boolean fusion) {
     ...
     */
    public static void removeFichierFromComposition(List<Composition> list, List<String> uuids,
            Predicate<Fichier> filterFile) throws MajorException {
    ...
    private static double getPercent(final BigDecimal bg, double percent) {
        return bg.divide(BigDecimal.valueOf(100D), RoundingMode.HALF_UP).multiply(BigDecimal.valueOf(percent))
                .doubleValue();
    }
}
